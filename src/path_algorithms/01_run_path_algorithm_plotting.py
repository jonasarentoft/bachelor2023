...
#[107386, 28219, 41438, 135921, 46909, 150934, 147721, 75, 148848, 103426]
    ...
    print(newMinLon, newMaxLon, newMinLat, newMaxLat)
    ...
    if specifiedAlgorithm == 'ch':
        aggS = cvs.line(source = positionsOfNodesInShortCutPathDF, x = 'x', y = 'y', line_width=7)#, line_width=5)
        pathS = ds.tf.shade(aggS, cmap=cc.kb)
        path = ds.tf.stack(path, pathS)
   
    xF = []
    yF = []
    gF = []
    
    for i, (k, v) in enumerate(forwardPrevious.items()):
        gF.append(i)
        gF.append(i)
        xF.append(lon[k])
        xF.append(lon[v])
    ...
    dfF = pd.DataFrame({'g':gF, 'x':xF, 'y': yF})
    dfF = dfF.set_index('g').groupby('g').apply(lambda x: x.reset_index(drop=True).reindex(np.arange(len(x) + 1)))
    aggF = cvs.line(source = dfF, x = 'x', y = 'y', line_width=2)#, line_width=5)
    searched = ds.tf.shade(aggF, cmap=cc.kg)
        
    if specifiedAlgorithm in ['bidirectional', 'bidirectional_alt', 'bidirectional_a_star', 'ch']:
        ...
        for i, (k, v) in enumerate(backwardPrevious.items()):
            gB.append(i)
            gB.append(i)
            xB.append(lon[k])
            xB.append(lon[v])
        ...
        dfB = pd.DataFrame({'g':gB, 'x':xB, 'y': yB})
        dfB = dfB.set_index('g').groupby('g').apply(lambda x: x.reset_index(drop=True).reindex(np.arange(len(x) + 1)))
        aggB = cvs.line(source = dfB, x = 'x', y = 'y', line_width=2)#, line_width=5)
        backwardSearched = ds.tf.shade(aggB, cmap=cc.kr)
        searched = ds.tf.stack(searched, backwardSearched)



    stacked = ds.tf.stack(img, searched, path)
    stacked = ds.tf.set_background(stacked, "white")
    #export_image(stacked, "out")
    stacked = stacked.to_pil()
